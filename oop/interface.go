...
func main() {
	...
	fmt.Println("x", x) // Hello
	fmt.Println("&x", &x)
	fmt.Println("x.name", x.name) // sam

	var x2 Animal
	x2 = Animal{name: "Chi Thong"}

	fmt.Println("x2", x2) // Hello
	fmt.Println("&x2", &x2)
	fmt.Println("x2.name", x2.name) // sam
...
// A implements Stringer interface with method String()
